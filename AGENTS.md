...
## Local Novel Repository Is Authoritative

- The default novel repository is `E:\claude\noval_base`; the active novel project inside it is `noval-result\`.
- The current local working tree is the source of truth for writing progress, including modified and untracked files. Determine progress from the local `noval-result\.agent\status.md`, active task files, chapters, archives, settings, and prompts.
- Git commits are local recovery snapshots. GitHub pushes are cloud backups. Neither `HEAD` nor `origin/main` defines the current writing progress when newer local work exists.
- Never use `git pull`, `git reset`, `git checkout`, `git restore`, `git clean`, rebase, or remote contents to overwrite or roll back local novel work unless the author explicitly requests that exact operation after reviewing the affected files.
- Do not commit or push unless the author explicitly asks. Always distinguish uncommitted working-tree progress, a local commit, and a GitHub push in reports.
- If local status files and local manuscript files disagree, preserve both, inspect the discrepancy, and ask the author before replacing either one.
...
**Proactive work you can do without asking:** read and organize memory files; inspect projects with read-only commands such as `git status`; update non-novel operational documentation; review and update `MEMORY.md`. Do not commit or push without the author's explicit request.
